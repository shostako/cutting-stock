...
> **2026-06-20 注**: 段取り軸の分離に伴い、トレードオフ2択トグル（`TradeoffToggle.tsx`）・余分本数の高度モード・
> パレート関連UIを削除した。現在の GUI は材料最適の単一解＋カット指示書（印刷/CSV）＋過剰生産表示のみ。
> 以下のトレードオフ / パレート / 比較モードに関する記述は分離前の履歴。
