...
> 辞書式の**単一解**（本数最小→切り方最小）。主な要素: 比率忠実な棒グラフ・カット指示書（印刷/CSV）・
> 2段の最適性証明バッジ・ラベル方式選択（長さ/英字/数字/手動）・デモ例（demo1-3）・ライト/ダーク切替（OS追従・記憶）・
> スマホ対応（≤1024pxで1カラム）・過剰生産は出た時のみ警告。開発者向け表記（記号L/k・モデル名・ソルバ名）は撤去済み。
