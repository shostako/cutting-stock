...
> **2026-06-20 注**: 現状の `solve()` は**辞書式2段**=① arc-flow MIP on HiGHS で本数最小（gap=0 証明）→
> ② `setup_mip.py` の CP-SAT 候補プール選択MIP で本数固定のまま切り方最小（証明付き）。パレート掃引（旧 `pareto.py`）は
> 廃し単一解を返す。以下のパレート前線/ε制約に関する記述は旧設計の履歴。`oracle.py` は独立 CP-SAT で本数を裏取り（CI保険）。
...
3. **MIP**（`flow_mip.py`, HiGHS via `highspy`）: フロー保存（各内部頂点）+ 需要充足（`Σ item弧フロー == d_i`, 2026-07-02 に ≥ から締めた。過剰生産を作らない。余剰は loss 弧＝未カット端材で吸収でき z* 不変）。
...
## 2026-07-02: 需要制約を == に統一（過剰生産の廃止）

**動機**: ≥ 制約下では両段とも「切らなくてもよいピース」を切る解が出得る（特に旧主経路の
maximal 限定プールは構造的に過剰生産を製造していた）。現場的には余剰は切らず端材のまま残すのが正:
廃棄量の定義（`z·L − 総需要長`）は不変で、余計なカット工数と誤解を招く「過剰生産」表示だけが消える。

**変更**:
- `flow_mip.py`: `Σ item弧フロー == d_i`。任意の ≥ 解は余剰ピースを loss 弧（未カット端材）に
  置換して == 解にできるため z* は不変（loss 弧は全頂点→sink に張ってあり、canonical order 上
  任意の部分multisetパターンが経路として表現可能）。
- `setup_mip.py` 主経路: == では maximal 置換論法が使えないため、プールを**全有効パターン**
  （`Σw ≤ L`, 非空）の列挙に拡大（`_enumerate_patterns`）。プール＝全パターンゆえ proven の
  正当性はむしろ単純化。材料解（== を満たす）を warm-start ヒント注入。cap(5万) 超は config-B 退避。
- `_solve_configb`: 需要 `== d_j` に変更。seed は本数一致時のみ採用（== を壊す調整を廃止）。

**検証**: Wikipedia 例題は == でも 73本 / 1640 / **P=10 proven** を維持（既知最小と一致、
`test_wikipedia_lexicographic_optimum` を「需要ちょうど」の assert に強化）。全テスト green。
GUI のランダムデモ連続実行でも過剰生産テーブルは不出現（Playwright で確認）。
