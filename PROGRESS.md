...
5. **M2 正当性検証（ウルトラコード）**: `oracle.py`（CP-SAT 割当直接解）を実装し arc-flow と目的値一致を CI 化。
   BPPLIB（Falkenauer U/T, Hard28）取込・既知最適突合。kerf不変条件 property test。これが緑になるまで段取り軸（M3）に進まない。
